...
| **F11** | ✅ **Tamam** | İstemci `X-Timezone-Offset` (dk) gönderiyor; `ICurrentUserService.UtcOffsetMinutes` (default member) okur; DashboardService + StatisticsService + StatisticsController tüm `ToLocalTime()`/`DateTime.Today`/`DateTime.Now` yerine kullanıcı offset'ini kullanıyor. **Canlı doğrulandı** (offset değişince gün dağılımı/"bugün" kayıyor, Türkiye=sunucu-yerel → regresyon yok). Test: `TimezoneOffsetTests`. |
| **F07** | ✅ (F11 kapsamı) | Dashboard "bugün" C#'ta offset ile yeniden hesaplandığından view'ın UTC `TodayStudyMinutes` kolonu zaten kullanılmıyor; görünür değer doğru. |
...
| **B2** | **Top 5 görev grafiği boş** (mobil + desktop) | Grafik yalnız **tamamlanmış VE TaskId ile başlatılmış** oturuma sahip görevleri gösteriyordu; oturumlar nadiren task'a bağlandığından kesişim genelde boş. Ayrıca offline'da temp→server task-id reconciliation eşleşmeyi koparıyordu. | Server ([StatisticsService](StudyTime.Application/Services/StatisticsService.cs)) + offline ([LocalSessionAnalytics](StudyTime.DesktopClient/Offline/LocalSessionAnalytics.cs)): "tamamlanmış" şartı kaldırıldı → **süreye göre** sıralanıyor; offline'da TaskId temp→server eşlemesi uygulanıyor. UI rozeti "Süreye göre", satır ikonu tamamlanma durumuna göre. Test: `TopTasksStatisticsTests`. **Canlı doğrulandı** (5 görev, tamamlanmamışlar dahil). |
| **B3** | Manuel çıkışta login ekranında yanlış **"başka cihazda açıldı"** mesajı | Çıkış sunucu hwid'ini siler; o an uçuştaki arka plan istekleri eski hwid ile gidip `SESSION_MISMATCH` 401 alır; handler bunu "başka cihaz" sanar. | `UserInitiatedLogout` bayrağı: manuel çıkışta set edilir, girişte sıfırlanır; handler kullanıcı kendi çıktıysa mismatch'i "başka cihaz" saymaz; stale `LoginError` temizlenir. Gerçek "başka cihaz" tespiti korunur. **Canlı doğrulandı.** |
